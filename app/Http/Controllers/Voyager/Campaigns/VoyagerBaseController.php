...
namespace App\Http\Controllers\Voyager\Campaigns;
...
class VoyagerBaseController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if($start_date->lte($now)){
                ...
                return redirect()->back()->with([
                    'message'    => __('Campaign already has started, You cannot edit it !!!!!'),
                    'alert-type' => 'error',
                ]);
        }

        // if(isset($request['giftsNames'])){
        //    dd("dlfmvjszlfi");
        //     $giftsNames = $request['giftsNames'];
        //     $giftsNames = array_values($giftsNames);
        //     $as[] = json_encode($giftsNames,true);
        //     foreach ($as as $key => $values){
        //         $new_gifts = [];

        //         $values = json_decode($values,true);
        //         foreach ($values as $key => $value){
        //                if(in_array($value, $new_gifts)){

        //                    return redirect()->back()->with([
        //                      'message'    => __('You can not insert two distributors with the same name'),
        //                      'alert-type' => 'error',
        //                     ]);
        ...
        //     $dis[] = json_encode($distributors,true);
        //     foreach ($dis as $key => $values){
        //         $new_dis = [];

        //         $values = json_decode($values,true);
        //         foreach ($values as $key => $value){
        //                if(in_array($value, $new_dis)){
        ...
        if(isset($input['start_date'])){
            ...
            if($start_date->lt($now)){
                ...
                return redirect()->back()->with([
                    'message'    => __('start date must be greater or equal today'),
                    'alert-type' => 'error',
                ]);
        ...
        if(isset($input['gain_start_date'])){
            $gain_start_date = Carbon::parse($input['gain_start_date']);
            if($gain_start_date->lt($start_date) || $gain_start_date->gt($end_date)){
                
                return redirect()->back()->with([
                    'message'    => __('gain start date must be greater or equal start date and less or equal end date'),
                    'alert-type' => 'error',
                ]);
            }
        ...
        if(isset($input['gain_end_date'])){
            $gain_end_date = Carbon::parse($input['gain_end_date']);
            if($gain_end_date->lt($gain_start_date) || $gain_start_date->gte($end_date)){
                
                return redirect()->back()->with([
                    'message'    => __('gain end date must be greater than gain start date and less or equal end date '),
                    'alert-type' => 'error',
                ]);
            }
            ...
            foreach ($as as $key => $values){
                ...
                foreach ($values as $key => $value){
                       if(in_array($value, $new_gifts)){
                           ...
                           return redirect()->back()->with([
                             'message'    => __('You can not insert two gifts with the same name'),
                             'alert-type' => 'error',
                            ]);
    ...
    public function destroy(Request $request, $id)
    {
        ...
        foreach ($ids as $id) {
            ...
            if($start_date->lte($now)){
                return redirect()->back()->with([
                    'message'    => __('you can not delete this campaign'),
                    'alert-type' => 'error',
                ]);
            }
            // Check permission
